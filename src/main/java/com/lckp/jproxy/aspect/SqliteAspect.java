...
public class SqliteAspect {
	...
	public Object around(ProceedingJoinPoint pjp) throws Throwable {
		...
		log.debug("切入点：{}", pjp.getTarget().getClass().getName());
		if (name.contains("save") || name.contains("update") || name.contains("remove")) {
			log.debug("{} 开始获取写锁", name);
			readWriteLock.writeLock().lock();
			try {
				log.debug("{} 获得了写锁", name);
				return pjp.proceed();
			} catch (Exception e) {
				...
				log.debug("{} 释放了写锁", name);
			}
		} else {
			log.debug("{} 开始获取读锁", name);
			readWriteLock.readLock().lock();
			try {
				log.debug("{} 获得了读锁", name);
				return pjp.proceed();
			} catch (Exception e) {
