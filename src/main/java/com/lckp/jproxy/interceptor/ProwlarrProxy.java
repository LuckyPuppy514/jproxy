...
/**
 * @className: ProwlarrProxy
 * @description: prowlarr代理
 * @date 2022年6月17日
 * @author LuckyPuppy514
 */
public class ProwlarrProxy extends ApiProxy implements HandlerInterceptor {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ProwlarrProxy.class);
	
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		ApiConfig apiConfig = (ApiConfig) ApplicationContextHolder.getBean("apiConfig");
		LOGGER.info("proxy prowlarr: {}", apiConfig.getJackett());

		ProxyParam proxyParam = new ProxyParam();
		proxyParam.setUrl(apiConfig.getProwlarr());
		proxyParam.setSearchKeyField(Proxy.PROWLARR_SEARCH_KEY_FIELD);
